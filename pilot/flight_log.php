...
$stmt = $pdo->prepare("
    ...
    WHERE fr.id = ? AND fr.pilot_id = ?
");
$stmt->execute([$id, $pilotId]);
$report = $stmt->fetch();

if (!$report) {
    echo "Relatório não encontrado ou acesso negado.";
    exit;
}
...
$pageTitle = "Log de Voo " . $report['flight_number'] . " - SkyCrew OS";
$extraHead = '
...
<div class="flex-1 flex flex-col space-y-6 overflow-hidden">
    ...
    <div class="flex justify-between items-center shrink-0">
        <div class="flex items-center gap-4">
            <a href="logbook.php" class="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 hover:text-white transition">
            ...
            <div>
                ...
                <p class="text-[10px] text-slate-500 uppercase tracking-widest mt-1">
                    Análise Individual de Telemetria • PIREP #<?php echo $report['id']; ?>
                </p>
            </div>
        </div>
        <div class="flex gap-3">
             <div class="bg-indigo-500/10 border border-indigo-500/20 px-4 py-2 rounded-2xl text-center">
                <p class="text-[9px] font-bold text-indigo-400 uppercase tracking-widest mb-1">Ganhos</p>
                <p class="text-xl font-black text-white">R$ <?php echo number_format($report['pilot_pay'], 2, ',', '.'); ?></p>
            </div>
             <div class="bg-blue-500/10 border border-blue-500/20 px-4 py-2 rounded-2xl text-center">
                <p class="text-[9px] font-bold text-blue-400 uppercase tracking-widest mb-1">Pontos</p>
                <p class="text-xl font-black text-white"><?php echo $report['points']; ?></p>
            </div>
        </div>
    </div>
    ...
    <div class="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 overflow-hidden">
        ...
        <div class="glass-panel rounded-3xl flex flex-col overflow-hidden">
            <div class="p-4 border-b border-white/10 bg-white/5">
                <h3 class="text-sm font-bold text-white uppercase tracking-widest">Eventos do Voo</h3>
            </div>
            <div class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                <?php if (empty($events)): ?>
                    <p class="text-center text-slate-500 italic text-sm">Nenhum dado de evento disponível.</p>
                <?php else: ?>
...
<script>
    ...
    const map = L.map('flightMap', { zoomControl: false, attributionControl: false });
    ...
    if (events.length > 0) {
        ...
        events.forEach((ev, i) => {
            if (ev.Latitude && ev.Longitude) {
                ...
                if (i === 0 || i === events.length - 1 || ev.IsError || ['Takeoff', 'Approach', 'Landing'].includes(ev.Phase)) {
                    ...
                    L.circleMarker(pos, {
                        ...
                        weight: 2,
                        fillOpacity: 1
                    }).addTo(map).bindTooltip(`<b>${ev.Phase}</b><br>${ev.Message}`, { direction: 'top', className: 'glass-tooltip', offset: [0, -5] });
                }
            }
        });

        if (path.length > 0) {
            L.polyline(path, { color: '#6366f1', weight: 3, opacity: 0.6, dashArray: '5, 10' }).addTo(map);
